...
public class GetCustomersTaskHandlerTests
{
    ...
    public async Task HandleAsync_Returns_Completed_Result_With_Matching_TaskId()
    {
        ...
        repoMock
            .Setup(r => r.GetCustomersAsync(It.IsAny<DateTime>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<CustomerDto> { new() { CustomerId = 1, FirstName = "Jane", LastName = "Doe" } });

        var sut = new GetCustomersTaskHandler(repoMock.Object, NullLogger<GetCustomersTaskHandler>.Instance);
        var task = CreateTask();
    ...
    public async Task HandleAsync_Falls_Back_Gracefully_When_ModifiedSince_Missing()
    {
        var repoMock = new Mock<IAdventureWorksRepository>();
        repoMock
            .Setup(r => r.GetCustomersAsync(DateTime.MinValue, It.IsAny<CancellationToken>()))
        ...
        Assert.Equal("completed", result.Status);
        repoMock.Verify(r => r.GetCustomersAsync(DateTime.MinValue, It.IsAny<CancellationToken>()), Times.Once);
    }
}
